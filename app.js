...
/* Render functions with birthday handling */
function renderMonth(){
  ...
  for(let d=1;d<=lastDate;d++){
    ...
    const hasEvents = (events[ds]||[]).length>0;
    ...
    html+=`<div class="day-cell${holiday?' holiday':''}" data-date="${ds}" ${holiday?`title="${holiday.label}"`:''}>
            ...
            ${(events[ds]||[]).map(ev=>`<div class="day-dot ${ev.yearly?'birthday':''}" title="${ev.text}"></div>`).join('')}
            </div></div>`;
  }
...
function renderWeek(){
  const start=new Date(currentDate); const dayIndex=(start.getDay()+6)%7; start.setDate(start.getDate()-dayIndex);
  let html=`<div class="week-grid"><div></div>`; const days=[];
  ...
  for(let h=0;h<24;h++){ html+=`<div class="hour-label">${pad(h)}:00</div>`; for(let i=0;i<7;i++){ const d=days[i]; const ds=`${d.getFullYear()}-${d.getMonth()+1}-${d.getDate()}`; html+=`<div class="hour-cell" data-date="${ds}" data-hour="${h}"></div>`; } }
  html+=`</div>`; views.week.innerHTML=html;

  for(const ds in events){ (events[ds]||[]).forEach(ev=>{ const hour=ev.hour; const sel=hour!==undefined?`.hour-cell[data-date="${ds}"][data-hour="${hour}"]`:`.hour-cell[data-date="${ds}"][data-hour="0"]`; const cell=document.querySelector(sel); if(cell){ const el=document.createElement('div'); el.className='event'; el.textContent=ev.text; cell.appendChild(el); } }); }
...
function renderDay(){
  ...
  const ds=`${y}-${m+1}-${d}`; let html=`<div style="display:grid;grid-template-columns:60px 1fr">`;
  ...
  (events[ds]||[]).forEach(ev=>{
    const hour = ev.yearly?0:ev.hour;
    const sel=`.day-hour[data-date="${ds}"][data-hour="${hour}"]`;
...
function renderYear(){
  const y=currentDate.getFullYear(); let html=`<div class="year-grid">`;
  for(let m=0;m<12;m++){ const monthStart=new Date(y,m,1); const monthName=monthStart.toLocaleString(undefined,{month:'long'}); html+=`<div class="year-month" data-month="${m}" data-year="${y}"><strong>${monthName}</strong><div style="font-size:12px;margin-top:6px">${renderMiniMonth(y,m)}</div></div>`; }
  html+=`</div>`; views.year.innerHTML=html;

  document.querySelectorAll('.year-month').forEach(el=>el.addEventListener('click',()=>{ const yy=parseInt(el.dataset.year,10), mm=parseInt(el.dataset.month,10); currentDate=new Date(yy,mm,1); setView('month'); }));
}

function renderMiniMonth(y,m){
  const first=new Date(y,m,1), firstIdx=(first.getDay()+6)%7;
  const lastDate=new Date(y,m+1,0).getDate(); const holidays=germanHolidays(y);
  let mini='<div style="display:grid;grid-template-columns:repeat(7,1fr);font-size:11px">';
  for(let i=0;i<firstIdx;i++) mini+='<div></div>';
  for(let d=1;d<=lastDate;d++){ const dt=new Date(y,m,d); const ds=`${y}-${m+1}-${d}`; const holiday=holidays.find(h=>h.date===ds); const evs=(events[ds]||[]).filter(ev=>ev.yearly); mini+=`<div style="padding:1px;${holiday?'color:#e84545;font-weight:bold;':''}">${d}${evs.map(ev=>` •`).join('')}</div>`; }
...
addEventBtn.addEventListener('click',()=>{
  ...
  let useHour=eventHourCheckbox.checked?parseInt(eventHourSelect.value,10):modalOpenFor.hour;
  const isBirthday=eventRepeatYearly.checked;
  if(isBirthday) useHour=undefined;
  if(!events[dateStr]) events[dateStr]=[];
  const ev=isBirthday?{text:txt,yearly:true}:{text:txt,hour:useHour};
  events[dateStr].push(ev);

  /* Add birthday for each year (adjust 29.02 ->28.02 if needed) */
  if(isBirthday){
    const parts=dateStr.split('-').map(Number);
    for(let y=currentDate.getFullYear()+1;y<=currentDate.getFullYear()+5;y++){ // future 5 yrs
      let m=parts[1],d=parts[2];
      if(m===2 && d===29 && !isLeapYear(y)) d=28;
      const ds=`${y}-${m}-${d}`;
      if(!events[ds]) events[ds]=[];
      events[ds].push({text:txt,yearly:true});
    }
  }
...
function adjustRowHeight(){
  const footer=document.querySelector('.footer'); const footerRect=footer?footer.getBoundingClientRect():{height:0};
  const available=window.innerHeight-footerRect.height; const weekdays=document.querySelector('.weekdays'); const weekdaysH=weekdays?weekdays.getBoundingClientRect().height:0;
  const rows=6; const rowHeight=Math.max(60,Math.floor((available-weekdaysH-8)/rows));
