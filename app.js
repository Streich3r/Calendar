/* app.js - Final, komplett, kommentiert
   - Verwende zusammen mit dem zuletzt gelieferten index.html + styles.css
   - Wichtig IDs in index.html müssen übereinstimmen:
     eventBirthdayCheckbox, eventHourCheckbox, eventHour, eventInput, addEventBtn, closeModalBtn, eventList, modalDate
*/
...
/* -------------------- Birthdays recurring --------------------
   - Birthdays stored once as events with birthday:true under the chosen date key
   - For any displayed date we compute which birthdays should appear (match by month/day)
   - Feb 29 birthdays -> shown on Feb 28 when year is not leap
   - Deduplicate by lowercased text when showing (so same name stored once is not repeated)
-----------------------------------------------------------------*/
...
function birthdaysForDateKey(ds){
  ...
  all.forEach(b=>{
    let bd = b.od, bm = b.om;
    if(bm===2 && bd===29 && !isLeap(y)) bd = 28; // fallback for non-leap years
    if(bm===m && bd===d){
...
/* -------------------- Week --------------------
   Grid layout:
   - top row: empty spacer + 7 weekday headers
   - second row: "All Day" label + 7 all-day stacked containers (class all-day-row)
   - following: hourly rows
-----------------------------------------------------------------*/
function renderWeek(){
  ...
  // all-day row label + cells (stacked)
  ...
  days.forEach(d=>{
    ...
    // stored all-day events (non-birthday, no hour)
...
/* -------------------- Year (3x4 mini-months) --------------------
   - 12 mini-months, grid 3x4
   - each mini-month is a little grid 7 cols (Mon..Sun)
   - append small dots under numbers for events/birthdays (centered)
   - no red dot for holiday; holiday shows as red number
-----------------------------------------------------------------*/
function renderYear(){
  ...
  for(let mm=0; mm<12; mm++){
    ...
    for(let d=1; d<=lastDate; d++){
      ...
      // dots: event & birthday small centered under number (no red dot for holiday)
      let dots = '';
      if(evs.length) dots += `<span class="dot event"></span>`;
      if(bds.length) dots += `<span class="dot birthday"></span>`;
      if(dots) html += `<div class="mini-dots" aria-hidden="true">${dots}</div>`;
      html += `</div>`;
    }
    html += `</div></div>`;
  }
  ...
  // click mini-month -> open month view on that month
...
function renderEventList(dateStr){
  ...
  bdays.forEach((b, idx)=>{
    ...
    del.addEventListener('click', ()=>{
      // remove stored birthday(s) that match the text (case-insensitive)
  ...
  // show normal events for this exact date
...
/* Toggle hour select; if birthday is checked, disable hour selector */
...
if(eventBirthdayCheckbox){
  eventBirthdayCheckbox.addEventListener('change', ()=>{
    if(eventBirthdayCheckbox.checked){
      // disable hour selection when marking birthday
...
todayBtn.addEventListener('click', ()=>{
  ...
  // requirement: today button always goes to MONTH view and highlights today
  setView('month');
});

/* Prev / Next behaviour (keeps current view) */
